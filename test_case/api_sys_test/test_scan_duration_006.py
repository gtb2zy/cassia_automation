...
'''
测试点： s系列：
            芯片0被动扫描，增加duration 12.3456s参数功能实现
		x/e系列：
		    芯片0被动扫描，增加duration 7.123s 参数功能实现；芯片1被动扫描，增加duration 0s参数功能，互相不影响，持续时间过后sse断掉
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E duration 参数测试chip0 被动扫 d=7.123s chip1 被动扫 d=0s，S chip0 被动扫 d=12.3456s')
...
def test_scan_filter_duplicates(self):
		...
		if self.local =="True":
			if self.model.startswith('S') or self.model.startswith('s'):
				a = threading.Thread(target=self.chip0_scan, args=(1, 12.3456))
				a.setDaemon(True)
				a.start()
				while True:
					if self.flag1 == 1:
				...
				a = threading.Thread(target=self.chip0_scan, args=(1, 7.123))
				b = threading.Thread(target=self.chip1_scan, args=(1, 0))
				a.setDaemon(True)
				b.setDaemon(True)
				b.start()
				a.start()
