...
测试点： s系列：
		...
		x/e系列：
		    芯片0被动扫描加不加filter duplicate过滤，芯片1主动扫描加filter duplicate 过滤功能，互相不影响，能正常过滤
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E 测试chip0  不加filter duplicate被动扫描 chip1 filter duplicate主动扫描，S chip0 filter duplicate 主动扫描')
