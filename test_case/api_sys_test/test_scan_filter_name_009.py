...
class testcase(unittest.TestCase):
    ...
    def test_scan_filter_name(self):
        if self.model.startswith('S') or self.model.startswith('s'):
            threading.Thread(target=self.chip0_scan,args = (1,self.filters['filter_name'])).start()
            threading.Thread(target=self.chip0_scan,args = (1,self.filters['filter_name'])).start()
            while True:
            ...
            threading.Thread(target=self.chip0_scan,args = (1,self.filters['filter_name'])).start()
            threading.Thread(target=self.chip1_scan,args = (1,self.filters['filter_name'])).start()
            while True:
                if self.flag1 and self.flag2:
                    self.assertTrue(True)
                    self.logger.info('pass\n')
