# -*- coding: utf-8 -*-
'''
测试点：测试chip0 chip1 filter name被动扫描，chip1 filter name 主动扫描
...
class testcase(unittest.TestCase):
    ...
    def test_scan_filter_name(self):
        if self.model.startswith('S') or self.model.startswith('s'):
            a = threading.Thread(target=self.chip0_scan,args = (0,self.filters['filter_name']))
            b = threading.Thread(target=self.chip0_scan,args = (0,self.filters['filter_name']))
            a.setDaemon(True)
            b.setDaemon(True)
            b.start()
            a.start()
            ...
            a = threading.Thread(target=self.chip0_scan,args = (0,self.filters['filter_name']))
            b = threading.Thread(target=self.chip1_scan,args = (0,self.filters['filter_name']))
            a.setDaemon(True)
            b.setDaemon(True)
            b.start()
            a.start()
