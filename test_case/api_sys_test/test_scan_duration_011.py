...
'''
测试点： s系列：
            芯片0被动扫描，增加duration 8.65s参数
		x/e系列：
		    芯片0被动扫描，不加duration 参数功能实现；芯片1被动扫描，增加duration -13.6参数功能，互相不影响，持续时间过后sse断掉
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E duration 参数测试chip0 被动扫 chip1 被动扫 d=-13.6s，S chip0 被动扫d=8.65s ')
...
def test_scan_filter_duplicates(self):
		...
		if self.local =="True":
			if self.model.startswith('S') or self.model.startswith('s'):
				a = threading.Thread(target=self.chip0_scan, args=(1, 8.65))
				a.setDaemon(True)
				a.start()
				while True:
					if self.flag1 == 1:
				...
				b = threading.Thread(target=self.chip1_scan, args=(1,-13.6 ))
				a.setDaemon(True)
				b.setDaemon(True)
				b.start()
				a.start()
