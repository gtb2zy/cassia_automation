...
'''
测试点： s系列：
            芯片0被动扫描，增加duration 0s参数功能实现
		x/e系列：
		    芯片0主动扫描，增加duration 5.78912s 参数功能实现；芯片1主动扫描，不加duration参数功能，互相不影响，持续时间过后sse断掉
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E duration 参数测试chip0 主动扫 d=5.78912s chip1 主动扫 ，S chip0 主动扫 d=0s')
...
def test_scan_filter_duplicates(self):
		...
		if self.local =="True":
			if self.model.startswith('S') or self.model.startswith('s'):
				a = threading.Thread(target=self.chip0_scan, args=(0, 0))
				a.setDaemon(True)
				a.start()
				while True:
					if self.flag1 == 1:
				...
				a = threading.Thread(target=self.chip0_scan, args=(0, 5.78912))
