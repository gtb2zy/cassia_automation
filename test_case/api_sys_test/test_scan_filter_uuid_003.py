...
测试点： s系列：
		...
		x/e系列：
		    芯片0主动扫描加filter uuid过滤，芯片1主动扫描不加filter uuid 过滤功能，互相不影响，能正常过滤
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E 测试chip0 filter uuid主动扫描 chip1 被动不加filter uuid，S chip0 filter uuid 被动扫描')
