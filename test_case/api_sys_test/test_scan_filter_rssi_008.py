...
'''
测试点： s系列：
            芯片0被动扫描，filter rssi不加信号强度过滤功能实现
		x/e系列：
		芯片0被动扫描加filter rssi过滤，芯片1被动扫描不加filter rssi 过滤功能，互相不影响，能正常过滤
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag1 = 0
		self.flag2 = 0
		self.logger.info('测试X/E chip0 filter rssi被动扫描 chip1 不加filter uuid 被动扫描，S chip0 不加filter uuid 被动扫描')
