...
'''
测试点： s系列：
            芯片0被动扫描，增加duration 6.789s参数功能实现
		x/e系列：
		    芯片0主动扫描，增加duration “abcde”参数功能实现；芯片1被动扫描，增加duration -1s参数功能，互相不影响，持续时间过后sse断掉
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E duration 参数测试chip0 主动扫 d="abcde" chip1 被动扫 d=-1s，S chip0 主动扫 d=6.789s')
...
def test_scan_filter_duplicates(self):
		...
		if self.local =="True":
			if self.model.startswith('S') or self.model.startswith('s'):
				a = threading.Thread(target=self.chip0_scan, args=(0, 6.789))
				a.setDaemon(True)
				a.start()
				while True:
					if self.flag1 == 1:
				...
				a = threading.Thread(target=self.chip0_scan, args=(0, "abcde"))
				b = threading.Thread(target=self.chip1_scan, args=(1, -1))
				a.setDaemon(True)
				b.setDaemon(True)
				b.start()
				a.start()
	...
	def chip0_scan(self, active=0, duration=None):
		...
		if duration:
			...
			# 小数位数小于等于3时，会按照设置的时间停止扫描；小数位数大于3时，参数不起效，一直扫描数据
				...
				duration1=duration
				print("整数和小数位数小于3 duration=", duration1)
		else:
			duration1=None
