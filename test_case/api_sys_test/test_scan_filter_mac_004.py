# -*- coding: utf-8 -*-
'''
测试点：测试chip0不加过虑主动扫描，chip1 filter mac 被动扫描
...
class testcase(unittest.TestCase):
    ...
    filter_count = int(read_job_config()['filter_count'])
    unfilter_count = int(read_job_config()['unfilter_count'])
    def setUp(self):
        self.timeout_flag = None
        self.flag1 = 0
        self.flag2 = 0
        self.logger.info('测试chip0不加过虑主动扫描，chip1 filter mac 被动扫描')
    ...
    def test_scan_filter_mac(self):
        if self.model.startswith('S') or self.model.startswith('s'):
            ...
            b = threading.Thread(target=self.chip0_scan, args=(0, self.filters['filter_mac']))
            a.setDaemon(True)
            b.setDaemon(True)
            b.start()
            a.start()
            ...
            a = threading.Thread(target=self.chip0_scan, args=(1,))
            b = threading.Thread(target=self.chip1_scan, args=(0, self.filters['filter_mac']))
            a.setDaemon(True)
            b.setDaemon(True)
            b.start()
            a.start()
