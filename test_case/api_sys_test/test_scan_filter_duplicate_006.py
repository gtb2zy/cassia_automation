...
'''
测试点： s系列：
            芯片0主动扫描，不加filter duplicate过滤功能实现
		x/e系列：
		    芯片0被动扫描加filter duplicate过滤，芯片1被动扫描filter duplicate 过滤功能，互相不影响，能正常过滤
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E 测试chip0  filter duplicate被动扫描 chip1 filter duplicate被动扫描，S chip0 不加filter duplicate 主动扫描')
    ...
    def test_scan_filter_duplicates(self):
		if self.model.startswith('S') or self.model.startswith('s'):
			...
			a = threading.Thread(target=self.chip0_scan, args=(1, 1))
