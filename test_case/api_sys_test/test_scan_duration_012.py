...
'''
测试点： s系列：
            芯片0被动扫描，增加duration 9.99s参数功能实现
		x/e系列：
		    芯片0被动扫描，不加duration 参数功能实现；芯片1主动扫描，增加duration 9.99参数功能，互相不影响，持续时间过后sse断掉
'''
...
class testcase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.flag2 = 0
		self.key=None
		self.logger.info('X/E duration 参数测试chip0 被动扫  chip1 主动扫 d=9.99s，S chip0 被动扫d=9.99s ')
	...
	def chip0_scan(self, active=0, duration=None):
		...
		with closing(self.sdk.scan(chip=0, active=active, duration=duration1)) as self.sse1:
			if duration1:
				...
				t=float(((d2-d1).seconds)) + float((d2-d1).microseconds/1000/1000) -1
				print("second",t)
				if t <=duration1:
					self.flag1 +=1
