        ...
        if code == 200:
            ...
            for i1 in range(0, len(txt)):
                if txt[i1]["connectionState"] == "connected":
                    print(" AP{0} get connected device:{1} success".format(self.conf['hub'],txt[i1]['bdaddrs']['bdaddr']))
                else:
                    loop_error = True
    ...
    def test_discover_service(self,device_conf):
        ...
        while not self.conn_flag:
            ...
            if code == 200:
                ...
                loop_error = True
                debug = 'ap={0} get services before get devlist failed,return code={1},msg={2}'.format(self.conf['hub'], code, text)
                self.debug.append(debug)
                # self.end_timer()
                break
    ...
    def test_discover_characteristics(self,device_conf):
        ...
        while not self.conn_flag:
            ...
            if code == 200:
                ...
                loop_error = True
                debug = 'ap={0} get characteristic before get devlist failed,return code={1},msg={2}'.format(self.conf['hub'],                                                                                                      code, text)
                self.debug.append(debug)
                # self.end_timer()
                break
