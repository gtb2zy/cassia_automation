...
def read_config():
	...
	'''
	try:
		path = os.getcwd().split('cassia_automation')[0]+'cassia_automation/config/config.json'
...
def read_job_config():
	...
	try:
		path = os.getcwd().split('cassia_automation')[0]+'cassia_automation/config/job_config.json'
		with open(path,encoding = 'utf8') as conf:
			conf = json.load(conf)
			if isinstance(conf,dict):
				logger.debug('Read config success.')
...
def read_stability_config():
	...
	try:
		path = os.getcwd().split('cassia_automation')[0]+'cassia_automation/config/environments.json'
		with open(path,encoding = 'utf8') as conf:
			conf = json.load(conf)
			if isinstance(conf,dict):
				logger.debug('Read config success.')
...
def get_device_list(model,ap):
	# 获取系统测试的的device_list
	try:
		path = os.getcwd().split('cassia_automation')[0] + 'cassia_automation/config/devices.json'
		with open(path, encoding='utf8') as conf:
